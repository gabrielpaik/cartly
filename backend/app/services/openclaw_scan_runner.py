...
def _raise_if_error_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ok = payload.get('ok')
    if ok is False:
...
def _parse_result_payload(payload: Dict[str, Any]) -> OpenClawScanResult:
    result = _raise_if_error_payload(payload)
...
def run_openclaw_scan(job_id: str, image_path: str) -> OpenClawScanResult:
    ...
    payload = _run_command(
        command_template.format(job_id=job_id, image_path=image_path),
        settings.openclaw_scan_timeout_seconds,
    )

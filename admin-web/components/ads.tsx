...
function buildSlotPreviewSpecs(
  t: (key: string, fallback?: string) => string,
): Record<string, SlotPreviewSpec> {
  return {
    save_complete_sheet_1: {
      screenTitle: t(
        "admin.ads.preview.spec.saveComplete.title",
        "Save Complete",
      ),
      screenSubtitle: t(
        "admin.ads.preview.spec.saveComplete.subtitle",
        "저장 완료 화면",
      ),
      placementLabel: t(
        "admin.ads.preview.spec.saveComplete.placement",
        "요약 아래, 액션 버튼 위",
      ),
    ...
    saved_inline_1: {
      ...
      screenSubtitle: t(
        "admin.ads.preview.spec.savedList.subtitle",
        "저장 카트 목록",
      ),
      placementLabel: t(
        "admin.ads.preview.spec.savedFirst.placement",
        "첫 카드 다음 위치",
      ),
    ...
    saved_inline_2: {
      screenTitle: t(
        "admin.ads.preview.spec.savedList.title",
        "지난 카트 리스트",
      ),
      screenSubtitle: t(
        "admin.ads.preview.spec.savedList.subtitle",
      ...
      placementLabel: t(
        "admin.ads.preview.spec.savedThird.placement",
        "세 번째 카드 다음 위치",
      ),
    ...
    my_perks_inline_1: {
      ...
      placementLabel: t(
        "admin.ads.preview.spec.my.placement",
        "계정 카드 아래",
      ),
      mode: "my-inline",
    },
    home_floating_1: {
      screenTitle: t("admin.ads.preview.spec.home.title", "Home"),
      screenSubtitle: t("admin.ads.preview.spec.home.subtitle", "홈 화면"),
      placementLabel: t(
        "admin.ads.preview.spec.homeFloating.placement",
        "하단 고정 영역 위 플로팅 시트",
      ),
      mode: "home-floating",
    },
...
function buildFallbackConfig(slotKey: string) {
  ...
  if (slotKey === "saved_inline_1") {
    return {
      ...
      slotDescription:
        "지난 카트 리스트 첫 카드 뒤에 들어가는 inline 광고 슬롯",
  ...
  if (slotKey === "saved_inline_2") {
    return {
      ...
      slotDescription:
        "지난 카트 리스트 세 번째 카드 뒤에 들어가는 inline 광고 슬롯",
...
function overlapsPeriod(
  ...
  const campaignStart = campaign.startAt
    ? new Date(campaign.startAt)
    : campaign.createdAt
      ? new Date(campaign.createdAt)
      : null;
  const campaignEnd = campaign.endAt
...
export function SlotEditorPanel({
  ...
  const currentTitle = isLive
    ? slot.config.title
    : (slot.config.reservedTitle ?? "");
  const currentMessage = isLive
    ? slot.config.message
    : (slot.config.reservedMessage ?? "");
  const currentCta = isLive
    ? (slot.config.ctaLabel ?? "")
    : (slot.config.reservedCtaLabel ?? "");
  const currentLink = isLive
    ? (slot.config.targetUrl ?? "")
    : (slot.config.reservedTargetUrl ?? "");
  const currentImage = isLive
  ...
  return (
    <div className="slotContentCard">
      <div className="sectionHeader" style={{ marginBottom: 14 }}>
        <div>
          <h3 className="panelTitle" style={{ marginBottom: 6 }}>
            {title}
          </h3>
      ...
      <div className="formGrid slotContentBody">
        ...
        {isLive ? (
          <label className="field">
            <div className="fieldLabel">
              {t("admin.ads.fields.slotLabel", "슬롯 이름")}
            </div>
            <input
              className="textInput"
              value={slot.config.slotLabel ?? ""}
              disabled={readOnly}
              onChange={(e) => onConfigChange({ slotLabel: e.target.value })}
            />
          </label>
        ) : null}
        ...
        {isLive ? (
          <label className="field">
            <div className="fieldLabel">
              {t("admin.ads.fields.slotDescription", "슬롯 설명")}
            </div>
            <textarea
              className="textInput"
              rows={3}
              value={slot.config.slotDescription ?? ""}
              disabled={readOnly}
              onChange={(e) =>
                onConfigChange({ slotDescription: e.target.value })
              }
            />
          </label>
        ) : null}
        ...
        {isLive ? (
          <label className="field">
            <div className="fieldLabel">
              {t("admin.ads.fields.placementNote", "노출 위치 설명")}
            </div>
            <input
              className="textInput"
              value={slot.config.placementNote ?? ""}
              disabled={readOnly}
              onChange={(e) =>
                onConfigChange({ placementNote: e.target.value })
              }
            />
          </label>
        ) : null}
            ...
            onChange={(e) =>
              onConfigChange(
                isLive
                  ...
                  : { reservedMessage: e.target.value },
              )
            }
          />
        </label>
            ...
            onChange={(e) =>
              onConfigChange(
                isLive
                  ...
                  : { reservedCtaLabel: e.target.value || null },
              )
            }
          />
        </label>
        ...
        <label className="field">
          ...
          <input
            ...
            onChange={(e) =>
              onConfigChange(
                isLive
                  ? { targetUrl: e.target.value || null }
                  : { reservedTargetUrl: e.target.value || null },
              )
            }
          />
        </label>
        ...
        <div className="scheduleEditorCard">
          <div className="fieldLabel">
            {isLive
              ? t("admin.ads.fields.exposureWindow", "노출 기간")
              : t("admin.ads.fields.reservationWindow", "예약 기간")}
          </div>
          <div className="scheduleGrid">
            <label className="field">
              <div className="scheduleFieldLabel">
                {isLive
                  ? t("admin.ads.fields.exposureStart", "노출 시작")
                  : t("admin.ads.fields.reservationStart", "예약 시작")}
              </div>
              <input
                type="datetime-local"
                className="textInput"
                value={
                  isLive
                    ? (slot.config.exposureStartAt ?? "")
                    : (slot.config.reservationStartAt ?? "")
                }
                disabled={readOnly}
                onChange={(e) =>
                  onConfigChange(
                    isLive
                      ? { exposureStartAt: e.target.value || null }
                      : { reservationStartAt: e.target.value || null },
                  )
                }
              />
            </label>
            <label className="field">
              <div className="scheduleFieldLabel">
                {isLive
                  ? t("admin.ads.fields.exposureEnd", "노출 종료")
                  : t("admin.ads.fields.reservationEnd", "예약 종료")}
              </div>
              <input
                type="datetime-local"
                className="textInput"
                value={
                  isLive
                    ? (slot.config.exposureEndAt ?? "")
                    : (slot.config.reservationEndAt ?? "")
                }
                disabled={readOnly}
                onChange={(e) =>
                  onConfigChange(
                    isLive
                      ? { exposureEndAt: e.target.value || null }
                      : { reservationEndAt: e.target.value || null },
                  )
                }
              />
            </label>
        ...
        <label className="field">
          <div className="fieldLabel">
            {t("admin.ads.fields.assetUpload", "배너 업로드")}
          </div>
          <label className="uploadBox">
            ...
            <div className="uploadTitle">
              {t("admin.ads.fields.assetSelect", "배너 파일 선택")}
            </div>
          ...
          {currentImage ? (
            <div className="saveMessage">
              {t("admin.ads.fields.connected", "연결됨:")} {currentImage}
            </div>
          ) : null}
        </label>
      </div>
      ...
      {currentImage ? (
        <div className="section" style={{ marginTop: 16 }}>
          <img
            src={currentImage}
            alt={`${slot.slotKey} ${variant} banner`}
      ...
      <div className="buttonRow slotPanelSaveRow">
        <button
          className="primaryBtn slotPanelSaveBtn"
          disabled={saving || readOnly}
          onClick={onSave}
        >
          {saving
            ? t("admin.ads.saving", "저장 중...")
            : isLive
              ? t("admin.ads.livePanel.save", "현재 광고 저장")
              : t("admin.ads.reservedPanel.save", "예약 광고 저장")}
        </button>
      </div>
    </div>
  );
    ...
    return (
      <div className="slotPreviewCard">
        <div className="slotPreviewMeta">
          <div className="slotPreviewTitle">
            {t("admin.ads.preview.title", "노출 위치 미리보기")}
          </div>
          <div className="slotPreviewText">
            {t(
              "admin.ads.preview.empty",
              "이 슬롯용 미리보기가 아직 준비되지 않았어",
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="slotPreviewCard">
      <div className="slotPreviewMeta">
        ...
        <div className="slotPreviewText">
          {preview.screenTitle} · {preview.screenSubtitle}
        </div>
        ...
        <div className="slotScheduleSummary">
          <div className="slotScheduleLine">
            <span>노출</span>
            <strong>
              {formatScheduleLabel(slot.config.exposureStartAt)} →{" "}
              {formatScheduleLabel(slot.config.exposureEndAt)}
            </strong>
          </div>
          <div className="slotScheduleLine">
            <span>예약</span>
            <strong>
              {formatScheduleLabel(slot.config.reservationStartAt)} →{" "}
              {formatScheduleLabel(slot.config.reservationEndAt)}
            </strong>
          </div>
        </div>
      </div>
      ...
      <div className="slotPreviewFrame">
        <div className="slotPreviewPhone">
          ...
          <div className="slotPreviewCanvas">
            ...
            {preview.mode === "home-floating" ? (
              <>
                <div className="slotPreviewBlock medium" />
                <div className="slotPreviewBlock medium" />
                <div className="slotPreviewBlock medium" />
                <div className="slotPreviewHighlight sheet">FLOATING AD</div>
                <div className="slotPreviewActions">
                  <div className="slotPreviewActionBtn ghost" />
                  <div className="slotPreviewActionBtn" />
                </div>
...
export function SlotHistory({
  ...
  const filtered = campaigns.filter((campaign) => {
    if (variantFilter !== "all" && campaign.variant !== variantFilter)
      return false;
  ...
  if (filtered.length === 0) {
    return (
      <div className="emptyState" style={{ marginTop: 16 }}>
        {t("admin.ads.history.empty", "조건에 맞는 지난 광고가 없어")}
      </div>
    );
  }
          ...
          {filtered.map((campaign) => (
            <tr key={campaign.id}>
              <td data-label={t("admin.ads.history.table.title", "광고명")}>
                ...
                <div className="adHistoryCellSub">
                  {campaign.message ||
                    t("admin.ads.history.noMessage", "메시지 없음")}
                </div>
              </td>
              ...
              <td data-label={t("admin.ads.history.table.period", "기간")}>
              ...
              <td data-label={t("admin.ads.history.table.image", "이미지")}>
                {campaign.imageUrl ? (
              ...
              <td
                data-label={t("admin.ads.history.table.download", "다운로드")}
              >
                <a

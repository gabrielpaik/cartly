...
- The Android test tracks are now aligned to `1.0.6 (40)` on both internal and alpha, so immediate Android release work is no longer binary/upload work. It is tester recruitment, active closed-test operation, and waiting out the Play-required test window.
...
3. Continue from the now-uploaded Android internal + alpha build `1.0.6 (40)` toward the Play closed-test requirement, including 12 opted-in testers and the 14-day gate.
